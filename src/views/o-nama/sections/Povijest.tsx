...
const Povijest = () => {
  return (
    <div className="pt-4 pb-8">
      ...
      <div className="m-auto grid max-w-screen-xl gap-4 px-3 lg:grid-cols-2 xl:grid-cols-3">
        {content.map(({ year, title, text }) => (
          <div className="rounded-md border-2 p-4 text-justify" key={title}>
            <div className="mb-4 border-b-2 pb-4 font-semibold">
              <span className="text-blue-500">{year}.</span> – {title}
            </div>
            <div>{text}</div>
          </div>
        ))}
